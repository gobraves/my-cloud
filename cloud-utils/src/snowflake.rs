...
impl SnowFlake {
    fn new(worker_id: i64, datacenter_id: i64) -> Self {
        ...
        if worker_id > max_worker_id || worker_id < 0 {
            panic!("worker_id can't be greater than {} or less than 0 ", max_worker_id);
        }

        if datacenter_id > max_datacenter_id  || datacenter_id < 0 {
            panic!("datacenter_id can't be greater than {} or less than 0 ", max_datacenter_id);
        }
    ...
    fn next_id(&mut self) -> i64 {
        ...
        if timestamp < self.last_timestamp {
            panic!("Clock moved backwards.  Refusing to generate id for {} milliseconds", self.last_timestamp - timestamp);
        }
